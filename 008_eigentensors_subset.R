#purpose: eigentensor analysis of g matrices for popsoil, pop, soil, sets of matrices. compared to randomized matrices. without low elevation pops
...
#remove malinalco and tepoztlan, the low elevation populations with some parv similarity. so. the 5th, 15th, and 25th
An.popsoil.5THE <- 	An.popsoil.5T[,,-c(5,10,15,20,25,30),]
An.popsoil.I5THE <- An.popsoil.I5T[,,-c(5,10,15,20,25,30),]
An.popsoil.N5THE <- An.popsoil.N5T[,,-c(5,10,15,20,25,30),]

#read in pedigrees, dataframes in list
load("~/GenoPheno/Popsoil_Peds.Rdata")#glm.popsoil.ped
glm.popsoil.pedHE <- glm.popsoil.ped[-c(5,10,15,20,25,30)] #rm tz and m

envmatall <-read.csv("~/GenoPheno/AlphabeticalPopEnvDat.csv")
envmatHE <- envmatall[-c(5,10),] #again rm low elevation pops
 
####RUN TENSOR ANALYSIS

#popsoil animal mods were iterated over 3, 5, 7 sample order of soil sites. but alphabetical order of plant pops
randAn.popsoil.5THE <- make.rand(An.popsoil.5THE,	glm.popsoil.pedHE,5,24,individs=30)#5 traits, 30 pops, 30 individuals per pop
randAn.popsoil.I5THE <- make.rand(An.popsoil.I5THE,	glm.popsoil.pedHE,5,24,individs=30)
randAn.popsoil.N5THE <- make.rand(An.popsoil.N5THE,	glm.popsoil.pedHE,5,24,individs=30)
...
#altered name vector, without malinalco OR tepoztlan
Gnames.ps <- paste(rep(c("cl","cu","da","fp","mc","mt","tc","tx"),times=3),".",rep(c("mc","mt","tc"),each=8),sep="")
Gnames <- Gnames.ps #covtensor requires Gnames and traitnames objects to exist
traitnames <- tnames5
tens.PS.5T <- covtensor(An.popsoil.5THE)
tens.PS.I5T <- covtensor(An.popsoil.I5THE)
tens.PS.N5T <- covtensor(An.popsoil.N5THE)
#has 15 tensors bc 15 gmat elements, which is less than 30 matrices

randtens.PS.5T <- covtensor(randAn.popsoil.5THE)
randtens.PS.I5T <- covtensor(randAn.popsoil.I5THE)
randtens.PS.N5T <- covtensor(randAn.popsoil.N5THE)
...
##WITHOUT LOW E POPS these are less significant. For the improper and nearly noninformative priors models, the first eigentensor is not sig at .95 CI, only .9,
# a mix of other eigentensors are sig, but, the variance explained is quite low except for the first two. (~45 & ~15 %)

pdf("~/GenoPheno/gmatrix/EigenValS_AnPopSoilG5trait_highEonly.pdf",height=4,width=4)
...
#var explained in first eigentensor almost exclusively first vector

rbd<-colorRampPalette(c(rgb(.8,0,0),rgb(.8,0,0,alpha=.5),rgb(1,1,1),rgb(0,0,.8,alpha=.5),rgb(0,0,.8)))

pdf("~/GenoPheno/gmatrix/TraitsPopSoilEigenVofEigenT5traits_highEonly.pdf",height=3,width=3)
par(mar=c(8,3,1,5))
...
pdf("~/GenoPheno/gmatrix/PopsoilTensor1.5trait_highEonly.pdf",height=4,width=4)
par(mar=c(9,9,1,1))
