...
Gnames.ps <- paste(rep(c("cl","cu","da","fp","m","mc","mt","tc","tx","tz"),times=3),".",rep(c("mt","mc","tc"),each=10),sep="")#alphabetical by pop but not soil
...
kde1 <- kde(sel.Anpopsoil.5T[,c(1,4),1], H=bandwithmat, approx.cont=T)
kde5 <- kde(sel.Anpopsoil.5T[,c(1,4),5], H=bandwithmat, approx.cont=T)
...
mtext("Lower Calimaya Plants",side=3,line=1,cex=1.5)
kde11 <- kde(sel.Anpopsoil.5T[,c(1,4),11],H=bandwithmat)
kde15 <- kde(sel.Anpopsoil.5T[,c(1,4),15],H=bandwithmat)
...
popmats <- matrix( rep(c(10,0,20),times=10) + rep(c(10,5,9,7,3,4,6,1,8,2),each=3), ncol=3,byrow=T) # all.sel.contours is alphabetical and soi is rearranged mc, mt, tc.
...
#	if(i==1){mtext("Selection for later flowering",side=3,line=2,cex=1.4)#}
	...
	legend(-3.75,.75,c("Biota15.0","Biota14.3","Biota13.0"),col=c(rgb(0,0,0),rgb(0,1,0),rgb(1,0,1)),pch=16,bty="n")
}
dev.off()
